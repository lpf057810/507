...
sf~crp: The existence of maternal age and/or gestational age (or pregnancy trimester) has a positive contribution to the association.

sfo~crp: The existence of maternal age and/or gestational age (or pregnancy trimester) has a positive contribution to the association.

si~crp: The existence of maternal age and/or gestational age (or pregnancy trimester) has a positive contribution to the association.

vitb12~crp: The existence of maternal age and/or gestational age (or pregnancy trimester)  has a positive contribution to the association.
...
for(micronutrient in micronutrients) {
  ...
  # Define the base model with no interactions
  model <- lm(as.formula(paste(micronutrient, "~ crp + age_yr + gagewks + trimester")), data = data)

  # Define models with individual interactions
  interaction_terms <- c("crp:age_yr", "crp:gagewks", "crp:trimester")
  models_with_interactions <- list(
    age_interaction = lm(as.formula(paste(micronutrient, "~ crp + age_yr + gagewks + trimester +", interaction_terms[1])), data = data),
    gagewks_interaction = lm(as.formula(paste(micronutrient, "~ crp + age_yr + gagewks + trimester +", interaction_terms[2])), data = data),
    trimester_interaction = lm(as.formula(paste(micronutrient, "~ crp + age_yr + gagewks + trimester +", interaction_terms[3])), data = data)
  )

  # Check each model for significance of interactions
  for (i in seq_along(models_with_interactions)) {
    model <- models_with_interactions[[i]]
    interaction_term <- interaction_terms[i]
    summary_model <- summary(model)
    print(summary_model)
    p_value <- coef(summary_model)[interaction_term, "Pr(>|t|)"]

    if (!is.na(p_value) && p_value < 0.05) {
      print(summary_model)
      cat(interaction_term, "interaction is significant for", micronutrient, "\n")
    } else {
      cat(interaction_term, "interaction is not significant for", micronutrient, "\n")
    }
  }
}
...
```{r}
model<-lm(si~crp + age_yr + gagewks + trimester + age_yr:crp + age_yr:gagewks + age_yr:trimester,data=data)
tab_model(model)
```
